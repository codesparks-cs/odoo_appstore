...
class SaleOrderLine(models.Model):
    ...
    def _onchange_discount_price(self):
        ...
        """
        for rec in self:
            if rec.discount_price <= (rec.product_uom_qty * rec.price_unit):
                if rec.discount_price == 0.00:
                    rec.update({
                        'discount': 0.00
                    })
                    ...
                    rec.update({
                        'discount': (rec.discount_price * 100)/(rec.product_uom_qty * rec.price_unit)
                    })
            elif(rec.discount_price > rec.price_subtotal):
                raise ValidationError(_(
                "Discounted amount for product '%(product)s' must be less than '%(amount)s'.",
    ...
    def _onchange_discount(self):
        for rec in self:
            rec.update({
                'discount_price': (rec.product_uom_qty * rec.price_unit * rec.discount) / 100
            })